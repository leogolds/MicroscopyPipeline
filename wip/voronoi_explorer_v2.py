...
def draw_voronoi_tiling(frame, quantiles):
    # print(quantiles)
    # exit(0)
    phase = h5py.File(base_path / "merged_binary_map.h5").get("exported_data")[
        # frame, 0, :, :, 0
        frame,
        ...,
    ]
    flourescence = h5py.File(base_path / "merged_binary_map.h5").get("exported_data")[
        # frame, 0, :, :, 0
        frame,
        ...,
    ]
...
quantiles_widget = pn.widgets.RangeSlider(
    name="Quantiles", start=0, end=1, value=(0.03, 0.93), step=0.01
)
